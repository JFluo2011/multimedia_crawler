...
class IQiYiSpider(scrapy.Spider):
    ...
    start_urls = [base_url.format(user) for user in users]
    ...
    def parse_params(self, response):
        ...
        url = host + src + '&vf=' + vf
        yield scrapy.FormRequest(url, method='GET', meta={'item': item}, callback=self.parse_video_urls)
    ...
    def parse_video_urls(self, response):
        ...
        for l in lst:
            item['media_urls'] = []
            url = url_prefix + l['l']
            yield scrapy.FormRequest(url, method='GET', meta={'item': item}, callback=self.parse_download_url)
    ...
    def parse_download_url(self, response):
        ...
        try:
            ...
            self.logger.error('url: {}, error: {}'.format(item['url'], str(err)))
            return
        item['media_urls'].append(json_data['l'])
